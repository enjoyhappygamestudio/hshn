-- Hải Sản Hà Nội — Seed Data (idempotent: ON CONFLICT DO NOTHING)
...
-- ========== CATEGORIES ==========
-- 012_categories.sql có thể đã thêm "Hải sản cấp đông" — seed bỏ qua nếu trùng id
...
SELECT v.product_id, v.label, v.price, v.unit, v.stock
FROM (VALUES
  ('c1b2c3d4-0003-4000-8000-000000000004'::uuid, '500g', 499000, '/500g', 12),
  ('c1b2c3d4-0003-4000-8000-000000000004'::uuid, '1 kg', 899000, '/kg', 5),
  ('c1b2c3d4-0003-4000-8000-000000000004'::uuid, '2 kg', 1690000, '/set', 2)
) AS v(product_id, label, price, unit, stock)
WHERE NOT EXISTS (
  SELECT 1 FROM product_variants pv
  WHERE pv.product_id = v.product_id AND pv.label = v.label
);
